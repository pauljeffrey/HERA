...
def _user_prompt(task: Task) -> str:
    return (
        ...
        f"Variation seed: {random.randrange(2**32)}."
    )
...
def _chat_completion_body(model: str, task: Task, response_format: dict) -> dict:
    body = {
        "model": model,
        "messages": [
    ...
    with path.open("w", encoding="utf-8") as f:
        for task in tasks:
            line = {
                ...
                "body": _chat_completion_body(model, task, response_format),
            }
    ...
    *,
    ...
    def _download(file_id: str, dest: Path) -> None:
        ...
        for attempt in range(max_retries):
            try:
                dest.write_bytes(client.files.content(file_id).read())
                return
    ...
    if batch.output_file_id:
        ...
        _download(batch.output_file_id, output_path)
    if batch.error_file_id:
        error_path = output_dir / f"{batch.id}_errors.jsonl"
        _download(batch.error_file_id, error_path)
...
def write_dataset(records: list[dict], path: Path) -> None:
    write_json_dataset(records, path, wrapper_key="patients")
