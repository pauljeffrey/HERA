...
EXTRACTED_FEATURES: dict[str, list[ExtractedFeature]] = {
    "PT-000001": [
        ExtractedFeature(
            ...
            pipeline_tier=PipelineTier.TIER_2_REGEX,
        ...
        ExtractedFeature(
            ...
            pipeline_tier=PipelineTier.TIER_1_BM25,
        ...
        ExtractedFeature(
            ...
            pipeline_tier=PipelineTier.TIER_3_LLM,
...
TRIAL_CRITERIA = [
    {
        "text": "LVEF ≤35% documented on echocardiography within 12 months",
        "is_inclusion": True,
        "keyword": "lvef",
        "threshold": 35,
    },
    {
        "text": "NYHA Class II-IV heart failure symptoms",
        "is_inclusion": True,
        "keyword": "nyha",
    },
    {
        "text": "Serum creatinine <2.0 mg/dL at screening",
        "is_inclusion": True,
        "keyword": "creatinine",
        "threshold": 2.0,
    },
    {
        "text": "Active myocardial infarction within 30 days",
        "is_inclusion": False,
        "keyword": "myocardial infarction",
        "negation_phrases": ["ruled out", "rule out", "negative"],
    },
    {
        "text": "Estimated GFR <30 mL/min/1.73m²",
        "is_inclusion": False,
        "keyword": "egfr",
        "threshold": 30,
    },
...
MOCK_AUDIT_PAYLOAD = HERA_AuditPayload(
    selected_patients=[
        EligiblePatient(
    ...
    search_space_after_tier1=10_000,
    search_space_after_tier2=1_000,
...
def get_patient_snapshot(patient_id: str, trial_id: str = TRIAL_ID) -> PatientSnapshotResponse | None:
    ...
    return PatientSnapshotResponse(
        ...
        audit_payload=MOCK_AUDIT_PAYLOAD if patient_id == "PT-000001" else HERA_AuditPayload(
            selected_patients=[],
...
def all_notes_flat() -> dict[str, str]:
    out: dict[str, str] = {}
    for pid, encounters in SOAP_NOTES.items():
        out[pid] = "\n\n".join(e.soap_note for e in encounters)
    return out
